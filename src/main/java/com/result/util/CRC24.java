...
public class CRC24 {
    ...
    public static final byte[] preamble = new byte[]{(byte) 211};

    public static byte[] crc24gen(byte[] data, int length, int before) {
        int result = before;
        for (int i = 0; i < length; i++) result = table[((result >> 16) ^ data[i]) & 0xFF] ^ (result << 8);
        return toByteArray(result);
    }

    public static byte[] crc24gen(byte[] data) {
        return crc24gen(data, data.length, 0);
    }

    public static int crc24gen(byte[] data, int before) {
        int result = before;
        for (int i = 0; i < data.length; i++) result = table[((result >> 16) ^ data[i]) & 0xFF] ^ (result << 8);
        return result;
    }
    ...
    /***
     * crc计算出报文对应的码，并用结果与 CRC字段进行比较，若一致，则校验成功
     * @param data  variable data部分的byte 数组
     * @param length 长度部分的byte数组
     * @return
     */
    public static byte[] crc24gen(byte[] data, byte[] length) {
        int result = 0;
        result = crc24gen(preamble, result);
        result = crc24gen(length, result);
        result = crc24gen(data, result);
        return toByteArray(result);
    }

    public static void checknumGen(byte[] data) {
        byte[] res = crc24gen(data, data.length - 3, 0);
        data[data.length - 3] = res[0];
        data[data.length - 2] = res[1];
        data[data.length - 1] = res[2];
        res = null;
    }
}
